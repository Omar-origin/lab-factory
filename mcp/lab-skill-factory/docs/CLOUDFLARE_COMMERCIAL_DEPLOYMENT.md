...
由可信测试者分别完成一笔 9.9 元体验版和一笔 49.9 元永久版交易：创建订单、扫码付款、提交交易信息、卖家核款、一键确认并自动发货、用户订单页领取密钥并激活。体验版还要验证前三个不同报告 ID 成功、重复 ID 不重复扣次、第四个 ID 被拒绝且 Skill 凝练入口被拒绝；永久版要验证不限次数并可凝练 Skill。退款验收完成后，确认客户端最迟在已有 24 小时租约到期时停用。
