...
description: |
  ...
  这个 skill 不直接代写实验报告，而是分析材料、生成 skill-spec.md，并通过 MCP 内置生成器或开发环境 skill-creator 结构规范生成/更新专属科目 skill，规定专属 skill 后续使用 fill.md + fill-map.json 的填补式报告流程。
---
...
- 生成出来的专属 skill 也必须遵循同一规则：每次处理新报告时先从头到尾读完整文档，复述任务要求并等待用户确认，再生成 `fill.md + fill-map.json`。
...
- 专属 skill 的报告流程必须是“填补式”：先生成 `fill.md`、`fill-map.json` 和必要的辅助产物计划给用户确认，再复制原文件并按锚点填入副本。
...
- 专属 skill 输出草稿后必须询问用户是否满意或有问题；有问题先按反馈修正，没问题则等待用户补齐截图/绘图/数据，并提醒用户完成后回来进行二次修改。
