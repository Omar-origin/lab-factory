...
下一阶段优先提升“材料 → 专属 Skill → 高质量草稿”的生成质量，而不是追求所有模板位置都自动命中。专属 Skill 必须让用户能看懂、能微调、能在草稿阶段反馈；定位不可靠时请求确认或阻断即可。每个生成的 Skill 都要包含 `subject-contract.md`、`skill-quality-contract.md` 和 `quality-profile.json`，把任务落地、结构完整、写作画像、报告差异化、证据完整、用户可调整性和隐私安全作为独立质量轴。
...
- 生成前提供一次简短的用户微调面板；用户可临时调整写作水平、内容详略、反思取向、语言语气和段落反馈，临时选择不自动沉淀为 Skill 规则。
